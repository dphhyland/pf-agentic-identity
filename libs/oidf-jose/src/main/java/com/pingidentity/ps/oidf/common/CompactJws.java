/*
 * Assembles a compact JWS from a claims payload and a JwsSigner.
 */
 ...
 *
 * <p>Exists so the several attestation minters in this codebase share one assembly path rather than
 * each re-deriving it. The signing input must be the exact bytes the header and payload serialise to —
 * re-serialising either after signing changes the signature — so the encoded parts are computed once
 * here and never rebuilt.
 *
 * <p>The header carries {@code alg}, {@code typ} and the signer's {@code kid}. Issuing keys are
 * referenced by id, never embedded: a verifier resolves the key through a trust path (a federation
 * chain, a pinned JWKS), not from the token asserting its own key.
 */
public final class CompactJws {
    ...
    /**
     * Signs {@code payloadJson} with {@code typ} in the header.
     *
    ...
    /** As {@link #sign(String, String, JwsSigner)}, with a caller-built header. */
    ...
    /**
     * As {@link #sign(Map, String, JwsSigner)}, serialising {@code payload} to JSON first — for callers
     * that hold their claims as a {@code Map} (e.g. a federation entity configuration) rather than a
