...
| `OIDFED §3(2)` | An entity statement without `typ: entity-statement+jwt` is rejected — the explicit-registration request body, and the entity configuration the attester reads its client bindings from | `ExplicitRegistrationRequest`, `OpenIdFederationClientResolver` | Implemented — `(2)` is §3's second paragraph (`#section-3-2`); a bare `OIDFED §3` row would count the `§3.1.x`/`§3.2` pins as covering it |
