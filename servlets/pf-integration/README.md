...
| `GET`/`POST /federation/admin/trust-marks` (`/revoke`, `/audit`) | `FederationAdminServlet` | Grants and revokes the Trust Marks this entity issues, lists grants by `sub` or `trust_mark_type`, and shows one grant's history. The admin bearer token (`OIDF_AUTHORITY_ADMIN_TOKEN`, constant-time; unset answers 401). Only a configured type, and a hosted-only type only to an active hosted entity. Every change records its actor - `admin:` and eight hex digits of the token's SHA-256, plus `X-Federation-Actor` when sent - in the grant's history and PF's audit log. Granting again revokes every mark minted under the grant before. |
