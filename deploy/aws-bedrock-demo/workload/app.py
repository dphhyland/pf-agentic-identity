"""EKS workload that authenticates to PingFederate with its IRSA identity.

The EKS analogue of the GKE agent. The pod mounts a projected service-account token with the attester as
its audience (see workload.yaml). That token is a JWT signed by the EKS cluster's OIDC issuer; the attester
validates it against the cluster's public JWKS and maps system:serviceaccount:<ns>:<sa> to a SPIFFE ID.
...
TOKEN_FILE = os.environ.get("EKS_TOKEN_FILE", "/var/run/secrets/tokens/attester-token")
...
def eks_projected_token() -> str:
    """The EKS IRSA evidence: a projected SA token, read fresh so kubelet rotation is picked up. Its
    audience is set to the attester on the pod's serviceAccountToken volume."""
...
def invoke(requested_details=None) -> dict:
    ...
    result = {"evidence_mode": "eks-sa-token", "mint_status": mint_status, "mint_body": mint_body}
    if mint_status != 200:
        return result
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        self._json(200, {"status": "up", "evidence_mode": "eks-sa-token"})
...
if __name__ == "__main__":
    print(f"[eks-workload] attester={ATTESTER_BASE_URL} token_file={TOKEN_FILE} · POST /invoke", flush=True)
